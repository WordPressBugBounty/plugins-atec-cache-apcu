...
return function($una)
{
	...
	$arr = ['Zlib' => $is_zlib_enabled, 'PC 🧂' => $salt];



	CPANEL::cpanel_header($una, __('Cached pages and posts', 'atec-cache-apcu'), [], '', $arr);
	...
	switch ($una->action)
		...
		$pattern = '/^atec_WPCA_' . preg_quote($salt, '/') . '_/';
		...
		TOOLS::table_header(
			...
			$reg_apcu = '/^atec_WPCA_' . preg_quote($salt, '/') . '_([fpcta]+)_([0-9]+(?:\|[0-9]+)*)$/';
			...
			foreach ($apcu_it as $entry)
			{
				...
				if (isset($match[2]))
				{
					...
					'<tr>';
						TOOLS::td($id);
						TOOLS::td(ucfirst($type));
						TOOLS::td($match[1].'_'.$match[2]);
						TOOLS::td($isCat ? $page : '');
						TOOLS::td(($isFeed ? ' <span class="'.esc_attr(TOOLS::dash_class('yes')).'"></span>' : ''));
						TOOLS::td($entry['num_hits']);
						TOOLS::td(size_format($entry['mem_size']), 'atec-nowrap');
						TOOLS::td($title);
						echo
			...
			if ($c>0) TOOLS::tr(['2@', number_format($c), '3@', TOOLS::size_format($size), '3@'], 'td', 'bold');
			else TOOLS::tr(['99@-/-']);
