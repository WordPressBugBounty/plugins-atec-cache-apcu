...
if (!function_exists('atec_plugin_settings')) { function atec_plugin_settings(array $links, $plugin): array { return ATEC_Admin::plugin_settings($links, $plugin); } }

class ATEC_Admin 
{
	public static function plugin_settings(array $links, $plugin): array 
	{
		$groupArr=[
		...
		preg_match('/([\w\-]+)\.php/', $plugin, $m);
		if (isset($m[1]))
		{
			$m = str_replace('atec-','',$m[1]);
			if (isset($groupArr[$m]))
			{
				$slug=$groupArr[$m];
